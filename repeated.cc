...
bool read_csv_file(std::string filename, ::person_info::Directory &people) {
    ...
    std::string read;
    while (fin >> read) {
        ::person_info::Person *person = people.add_person();
...
std::string create_filename(std::string filename) {
    ...
    std::string proto_file = absl::StrCat((std::string) absl::StrJoin(path, "/"), "/", name_extension.first, "_proto_repeated.", name_extension.second);
    return proto_file;
}

bool write_proto_file(std::string proto_file, ::person_info::Directory &people) {
    std::fstream fout(proto_file, std::ios::out | std::ios::binary);
    people.SerializePartialToOstream(&fout);
    return true;
}
...
bool read_proto_file(std::string proto_file) {
    ...
    ::person_info::Directory people;
    people.ParseFromIstream(&fin);
    for(auto person: people.person()) {
